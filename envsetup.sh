...
function build_build_var_cache()
{
    ...
    cached_vars=(`cat $T/build/envsetup.sh $T/vendor/euclid/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/_get_build_var_cached/) print $(i+1)}' | sort -u | tr '\n' ' '`)
    cached_abs_vars=(`cat $T/build/envsetup.sh $T/vendor/euclid/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/_get_abs_build_var_cached/) print $(i+1)}' | sort -u | tr '\n' ' '`)
...
function _lunch_meat()
{
    ...
    if [[ -z "${ANDROID_QUIET_BUILD}" && -z "${EUCLID_BUILD}" ]]; then
...
function lunch()
{
    ...
    then
        # if we can't find a product, try to grab it off the euclidOS GitHub
        T=$(gettop)
        cd $T > /dev/null
        vendor/euclid/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product $release
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/euclid/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
...
function setup_ccache() {
    ...
    if [ -z "${CCACHE_EXEC}" ]; then
        if command -v ccache &>/dev/null; then
            ...
            CCACHE_MAXSIZE="${CCACHE_MAXSIZE:-45G}"
...
. $ANDROID_BUILD_TOP/vendor/euclid/build/envsetup.sh
